...
def process_data_v12_8():
    ...
    # stats tính An, Gan cho con số (từ full dây) và Điểm (từ dây sạch)
    ...
    for wire_id, num in current_map.items():
        ...
        # 1. Luôn cập nhật An và Gan từ toàn bộ dây để phục vụ Bộ lọc
        ...
        # 2. CHỈ TÍNH ĐIỂM CHO DÂY SẠCH (streak_win == 0)
    ...
    for num, s in stats.items():
        ...
        # Thưởng nóng theo vùng An 0,1,2,3 (mày cần vùng này nên tao cộng mạnh)
        bonus = 0
        if s["max_an"] == 2: bonus = 5.0
        elif s["max_an"] == 3: bonus = 4.0
        elif s["max_an"] == 1: bonus = 2.0
        elif s["max_an"] == 0: bonus = 1.0
        final_score += bonus
...
# --- GIAO DIỆN ---
st.set_page_config(layout="wide", page_title="Matrix Final V12.8")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix Final V12.8</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if up_json and st.button("XÁC NHẬN NẠP"):
        ...
        st.session_state['history'] = data.get('history', []); st.session_state['last_full_str'] = data.get('last_full_str', "")
        process_data_v12_8(); st.rerun()
...
if st.session_state.get('df_raw') is not None:
    ...
    with c3: st.download_button("💾 XUẤT JSON V12.8", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_v12_8.json")
    ...
    with col_r:
        st.subheader("📜 LỊCH SỬ (Rank-An-Gan)")
