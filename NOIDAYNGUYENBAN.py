...
# --- 3. BỘ NÃO AI: THERMAL CORE (PURE 65%) ---
...
# --- 5. GIAO DIỆN CHUẨN ---
st.set_page_config(layout="wide", page_title="Matrix V13.60 Final")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix V13.60 - Lịch Sử & OCR Chuẩn</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with col2:
        if st.button("💎 KHỞI TẠO"):
            ...
            st.session_state['last_full_str'] = ""; process_matrix_v13_60(); st.rerun()
    ...
    if up_json and st.button("XÁC NHẬN NẠP"):
        ...
        st.session_state['last_full_str'] = data.get('last_full_str', ""); process_matrix_v13_60(); st.rerun()
    ...
    if not ai_on:
        ...
        if st.button("✅ ÁP DỤNG BỘ LỌC TAY"): process_matrix_v13_60(); st.rerun()
    ...
    if st.button("🔥 PHÂN TÍCH KỲ MỚI"):
        df_before = process_matrix_v13_60()
        ...
        if len(raw_list) >= 27:
            ...
            entry = {
                ...
                "AvgC": round(df_ai_current['Cứng(10k)'].mean(), 2) if not df_ai_current.empty else 0,
                "T5": get_t_rep(5), "T10": get_t_rep(10), "T15": get_t_rep(15), "T20": get_t_rep(20)
            }
            # Giữ các cột cũ từ JSON nếu có
            if st.session_state['history']:
                for k in st.session_state['history'][0].keys():
                    if k not in entry: entry[k] = "0"
            ...
            st.session_state['last_full_str'] = "".join(raw_list[:27]); process_matrix_v13_60(); st.rerun()
...
if st.session_state.get('df_raw') is not None:
    ...
    with col_d: st.download_button("💾 LƯU .JSON", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_v13_60.json")
    ...
    with c2: 
        ...
        if not h_df.empty:
            # Ưu tiên các cột STT, GĐB, Ai, AvgC lên đầu, các cột T sau
            cols = h_df.columns.tolist()
            fixed_cols = ["STT", "GĐB", "Ai", "AvgC"]
            t_cols = sorted([c for c in cols if c.startswith("T")], key=lambda x: int(x[1:]) if x[1:].isdigit() else 999)
            h_df = h_df[fixed_cols + t_cols]
