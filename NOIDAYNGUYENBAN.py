...
# --- 2. AI THERMAL CORE ENGINE (A:2,3 | T:1,2,3) ---
def get_thermal_ai_set(df_raw):
    ...
    def scoring(row):
        ...
        # Sửa Tầng 1, 2, 3 đồng hạng 5 điểm
    ...
    for _, r in rem.iterrows():
        ...
        # AI tự điều tiết nhiệt linh động
        if c_avg > 26: 
...
# --- 3. XỬ LÝ MA TRẬN (SYNC JSON DATA) ---
def process_matrix_v13_42():
    ...
    for w_id, num in c_map.items():
        w = db.get(str(w_id))
        if not w: continue
        s = stats[num]
        sw, sl = int(w.get("streak_win", 0)), int(w.get("streak_loss", 0))
        s["all_losses"].append(sl if sw == 0 else 0)
        if sw > s["max_an"]: s["max_an"] = sw
        s["clean_window_hits"] += sum(w.get("hit_history", [])[-WINDOW:])
        if sw == 0:
            s["clean_wire_count"] += 1
            s["total_score"] += float(w.get("score", 1000.0))
...
st.set_page_config(layout="wide", page_title="Matrix V13.42 Perfect Sync")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix V13.42 - Thermal Core & JSON Sync</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with c2:
        if st.button("💎 KHỞI TẠO"):
            ...
            process_matrix_v13_42(); st.rerun()
    ...
    if not ai_on:
        ...
        if st.button("✅ CHỐT DÀN"): process_matrix_v13_42(); st.rerun()
    ...
    if st.button("🔥 PHÂN TÍCH KỲ MỚI"):
        ...
        if df_now is not None:
            ...
            if len(raw_list) >= 27:
                ...
                df_f = get_thermal_ai_set(df_now) if ai_on else df_now.head(55)
                ai_l = df_f["Số"].tolist()
                ...
                st.session_state['last_full_str'] = "".join(raw_list[:27]); process_matrix_v13_42(); st.rerun()
...
if st.session_state.get('df_raw') is not None:
    ...
    with col_d: st.download_button("💾 LƯU .JSON", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_v13_42.json")
    st.code(", ".join(df_d.sort_values("Số")["Số"].tolist()))
    st.divider()
    c1, c2 = st.columns([1, 2.8])
    with c1: st.dataframe(df_d, use_container_width=True, hide_index=True)
    with c2: st.dataframe(pd.DataFrame(st.session_state['history']), use_container_width=True, height=800)
