...
def update_matrix_state(db, results_27, mapping):
    for wire_id, w_data in db.items():
        ...
        if num in results_27:
            ...
            hist = w_data.get("hit_history", [0]*20) # Lưu tối đa 20 kỳ để linh hoạt slider
...
# --- 3. BỘ NÃO PHỄU LỌC TRINITY CONTROL ---
def thermal_ai_engines_v75(df_raw, history, db, mapping, n_bottom, n_bet, hard_threshold):
    ...
    # Lọc Safe 79 dựa trên ngưỡng Cứng động từ Slider
    df_safe_orig = df_raw[df_raw['Cứng'] >= hard_threshold].sort_values(['Điểm', 'Rank'], ascending=[False, True]).head(79)
    # Nếu không đủ 79 con đạt ngưỡng Cứng, lấy bù cho đủ 79 theo Điểm
...
# --- 4. UI VÀ THANH TRƯỢT TỔNG ---
st.set_page_config(layout="wide", page_title="Matrix Total Control")
st.title("🛡️ Matrix V13.75 - Total Control Edition")
...
with st.sidebar:
    ...
    if up_json and st.button("XÁC NHẬN NẠP"):
        ...
        st.session_state['history'] = data.get('history', []); st.session_state['last_full_str'] = data.get('last_full_str', ""); st.rerun()

    st.divider()
    st.header("⚙️ ĐIỀU KHIỂN CHỈ SỐ")
    # Thanh trượt các chỉ số kỹ thuật mày yêu cầu
    val_tier = st.slider("Mật độ nén (Tầng %):", 50, 80, 65)
    val_window = st.slider("Cửa sổ soi nhịp (Kỳ):", 5, 20, 10)
    val_hard = st.slider("Ngưỡng Cứng tối thiểu (C%):", 0.0, 15.0, 8.0)
    
    st.divider()
    st.header("⚙️ ĐIỀU CHỈNH DÂY")
    ...
    if st.button("🔥 PHÂN TÍCH & LƯU"):
        ...
        if len(raw_list) >= 27 and gdb_input:
            ...
            st.session_state['history'].insert(0, {"STT": len(st.session_state['history']) + 1, "GĐB": gdb_input, "Dan39": check(p.get('d39', [])), "Dan59": check(p.get('d59', [])), "Dan79": check(p.get('d79', [])), "180thap": check(p.get('dthap', [])), "180cao": check(p.get('dcao', []))})
...
if st.session_state['last_full_str']:
    def get_matrix_df(t_val, w_val):
        ...
        avg_wires_dynamic = 11449 / 100 # Tính trung bình dây trên mỗi số
        ...
        for num, s in stats.items():
            ...
            hard = round((s["clean_window_hits"] / (w_val * (11449/1000))) * 100, 2) # Tính Hardness dựa trên Slider Window
            score = round((s["total_score"] / dc) * (1 + hard/100), 2)
            res.append({"Số": num, "Điểm": score, "Tang": calculate_tier(s["all_losses"], t_val), "DâySạch": s["clean_wire_count"], "Cứng": hard, "An": 0})
    ...
    c4, c5 = st.columns(2)
    c4.error(f"📉 Thấp {n_bottom_val} ({len(d_thap)})"); c4.code(", ".join(d_thap))
    c5.error(f"📈 Cao {n_bet_val} ({len(d_cao)})"); c5.code(", ".join(d_cao))

    st.divider()
    t1, t2 = st.tabs(["📜 LỊCH SỬ", "📊 CHI TIẾT"])
    with t1:
        if st.session_state['history']: st.dataframe(pd.DataFrame(st.session_state['history'])[["STT", "GĐB", "Dan39", "Dan59", "Dan79", "180thap", "180cao"]], use_container_width=True, hide_index=True)
    with t2:
