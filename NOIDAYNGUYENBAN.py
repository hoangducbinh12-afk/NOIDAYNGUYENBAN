...
# --- 2. BỘ NÃO AI: THERMAL BALANCE (LÀM VIỆC NGẦM) ---
def get_thermal_ai_set(df_raw, history):
    ...
    rem_df = df_raw[df_raw['AI_Score'] < 17].sort_values(['AI_Score', 'Điểm'], ascending=[False, False]).copy()
...
st.set_page_config(layout="wide", page_title="Matrix V13.33 Hybrid Control")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix V13.33 - AI & Manual Control</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with col2:
        if st.button("💎 KHỞI TẠO"):
            ...
            process_matrix_v13_33(); st.rerun()
    ...
    # --- THANH TRƯỢT THỦ CÔNG (Chỉ hiện khi TẮT AI) ---
    if not ai_on:
        ...
        if st.button("✅ ÁP DỤNG BỘ LỌC"): process_matrix_v13_33(); st.rerun()
    else:
        st.info("🤖 AI đang làm chủ: Purity 65%, Cân bằng nhiệt [20-28].")
    ...
    if st.button("🔥 PHÂN TÍCH KỲ MỚI"):
        df_now = process_matrix_v13_33()
        if df_now is not None:
            raw_list = [x.strip() for x in st.session_state['raw_input'].replace(",", " ").split() if x]
            if len(raw_list) >= 27:
                loto_list = [n[-2:] for n in raw_list[:27]]; gdb_val = st.session_state['gdb_val']
                df_final_ai = get_thermal_ai_set(df_now, st.session_state['history']) if ai_on else df_now[(df_now["Rank"] >= f_rank[0]) & (df_now["Rank"] <= f_rank[1]) & (df_now["An"] >= f_an[0]) & (df_now["An"] <= f_an[1]) & (df_now["Tang"] >= f_tang_min) & (df_now["Cứng(10k)"] >= f_hard[0]) & (df_now["Cứng(10k)"] <= f_hard[1])]
                ai_list = df_final_ai["Số"].tolist()
                ai_status = f"A({len(ai_list)})" if gdb_val in ai_list else f"T({len(ai_list)})"
                st.session_state['history'].insert(0, {"STT": len(st.session_state['history'])+1, "GĐB": gdb_val, "Ai": ai_status, "AvgC": round(df_final_ai['Cứng(10k)'].mean(), 2)})
                st.session_state['last_full_str'] = "".join(raw_list[:27]); process_matrix_v13_33(); st.rerun()

# --- 5. HIỂN THỊ KẾT QUẢ ---
if st.session_state.get('df_raw') is not None:
    df_raw_data = st.session_state['df_raw']
    if ai_on:
        df_display = get_thermal_ai_set(df_raw_data, st.session_state['history'])
    else:
    ...
    with col_m: st.metric("DÀN KẾT QUẢ", f"{len(df_display)} quân", f"AvgC: {df_display['Cứng(10k)'].mean():.2f}")
    with col_d: st.download_button("💾 LƯU .JSON", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_v13_33.json")
