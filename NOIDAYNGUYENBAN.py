...
def process_data_v12_0():
    ...
    for num, s in stats.items():
        ...
        # ĐIỂM GỐC VÀ THƯỞNG NÓNG VÙNG AN 2, 3
...
def audit_history(loto_list, gdb):
    ...
    stt_new = len(st.session_state['history']) + 1
...
# --- GIAO DIỆN ---
st.set_page_config(layout="wide", page_title="Matrix Final V12.0 Fix")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix Final V12.0</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if up_json and st.button("XÁC NHẬN NẠP"):
        ...
        process_data_v12_0(); st.rerun()

    st.divider()
    st.header("🎛️ BỘ LỌC")
    f_an = st.slider("An:", 0, 20, (0, 3)); f_gan = st.slider("Gan:", 0, 100, (0, 100))
    f_day = st.slider("Dây min:", 0, 115, 20); f_hard = st.slider("Cứng(10k) %:", 0.0, 100.0, 10.0, 1.0)
...
if st.session_state.get('df_raw') is not None:
    ...
    df_f = df_f[(df_f["An"] >= f_an[0]) & (df_f["An"] <= f_an[1]) & (df_f["Nén"] >= f_gan[0]) & (df_f["Nén"] <= f_gan[1]) & (df_f["Dây"] >= f_day) & (df_f["Cứng(10k)"] >= f_hard)].copy()
    ...
    with c3: st.download_button("💾 XUẤT JSON V12.0", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_final_v12_0.json")
