...
# --- 1. KHỞI TẠO CẤU HÌNH ---
TOTAL_POS = 107 
HANG_SO_DAY = 114.5 # Hằng số chuẩn hóa mật độ
...
def process_data_v11_4():
    ...
    # 1. Tính toán thống kê thô cho 100 số
    ...
    # 2. Áp dụng công thức Xếp hạng Nhân Hệ Số
    data_list = []
    for num, s in stats.items():
        # A. Độ cứng tổng (Thực lực quá khứ)
        if total_periods > 0 and s["wire_count"] > 0:
            do_cung_tong = (s["total_hits"] * HANG_SO_DAY) / (s["wire_count"] * total_periods)
        else: do_cung_tong = 0
            
        # B. Điểm trung bình gốc (Phong độ hiện tại từ DB)
        avg_score_db = s["total_score"] / s["wire_count"] if s["wire_count"] > 0 else 100.0
        
        # C. CÔNG THỨC NHÂN HỆ SỐ TRỌNG YẾU: Điểm gốc x (Cứng Tổng / 100)
        final_ranking_score = avg_score_db * (do_cung_tong / 100)
        ...
        data_list.append({
            ...
            "Nén": s["max_gan"], 
            "Dây": s["wire_count"], 
            "Cứng Tổng": round(do_cung_tong, 2)
        })
...
# --- 4. GIAO DIỆN ---
st.set_page_config(layout="wide", page_title="Matrix Final V11.4")
st.markdown("<h1 style='text-align: center; color: red;'>Matrix Final V11.4</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    st.header("📂 QUẢN TRỊ")
    if st.button("🚨 RESET TOÀN BỘ"): st.session_state.clear(); st.rerun()
    if st.button("💎 KHỞI TẠO MA TRẬN MỚI"):
    ...
    f_hard = st.slider("Cứng Tổng tối thiểu:", 0.0, 50.0, 25.0, 0.5)
    
    st.divider()
    st.header("📸 NHẬP KẾT QUẢ")
    up_img = st.file_uploader("Quét ảnh KQ", type=['jpg', 'jpeg', 'png'])
            ...
            if len(raw_list) >= 27:
                ...
                for i in range(11449):
                    ...
                    if num_f in loto_list:
                        ...
                        if wire["streak_win"] <= 3: wire["score"] += (1.0 * n_hits)
                        else: wire["score"] -= (1.0 * n_hits)
...
if st.session_state.get('df_raw') is not None:
    ...
    with c2: 
        st.write("**DÀN SỐ ĐÃ LỌC (Sắp xếp theo Số):**")
        st.code(", ".join(df_f.sort_values("Số")["Số"].tolist()) if not df_f.empty else "Trống")
    with c3: st.download_button("💾 XUẤT JSON V11.4", data=json.dumps({"matrix": st.session_state['db'], "history": st.session_state['history'], "last_full_str": st.session_state['last_full_str']}), file_name="matrix_final_v11_4.json")
    st.divider()
    col_l, col_r = st.columns([1, 2.5])
    with col_l:
        st.subheader("🎯 CHI TIẾT LỌC"); st.dataframe(df_f, use_container_width=True, height=450)
        with st.expander("📊 BẢNG TỔNG 100 SỐ FULL"): st.dataframe(df_all, use_container_width=True)
    with col_r:
        st.subheader("📜 LỊCH SỬ ĐỐI SOÁT V8.5"); st.dataframe(pd.DataFrame(st.session_state['history']), use_container_width=True, height=800)
